...
In a usual model lifecycle, a champion model will have been independently registered in your model inventory and submitted to you for validation by your model development team as part of the effective challenge process.

For this exercise, log in to the [ValidMind Platform](https://app.prod.validmind.ai) and register a sample model to circumvent the need for an existing model:

### Register a model

1. In the left sidebar, select **Inventory**.
2. Under the **RECORD TYPE** drop-down, select `Model` and click **+ Register Model**.
3. Enter the model details and click **Next >** to continue to assignment of inventory record stakeholders.
4. Select your own name under the **RECORD OWNER** drop-down — we'll adjust these permissions next for validation.
5. Click **Register Model** to add the model to your inventory.

### Assign validator credentials

On the model details page that appears after registration, swap your role from owner/developer to validator so that test results are logged under the validator role:
...
A template predefines sections for your report and provides a general outline to follow:
