...
**Import Path**: `validmind.vm_models.result.RawData`
...
**Import Path**: `validmind.tests.run.print_env`
...
```python
def combine_results(results: List) -> Tuple[, , ]
```
...
- **A tuple containing**: - A list of combined outputs (tables and figures). - A dictionary of inputs with lists of all values. - A dictionary of parameters with lists of all values.
...
```python
def get_comparison_test_configs(input_grid: Union = None, param_grid: Union = None, inputs: Union = None, params: Union = None) -> List[]
```
...
```python
def ClassImbalance(dataset: VMDataset, min_percent_threshold: int = 10) -> Tuple[, , bool]
```
...
```python
def DatasetSplit(datasets: List)
```
...
```python
def HighPearsonCorrelation(dataset: VMDataset, max_threshold: float = 0.3, top_n_correlations: int = 10, feature_columns: list = None)
```
...
```python
def MutualInformation(dataset: VMDataset, min_threshold: float = 0.01, task: str = 'classification')
```
...
```python
def ScoreBandDefaultRates(dataset: VMDataset, model: VMModel, score_column: str = 'score', score_bands: list = None)
```
...
```python
def ClassifierThresholdOptimization(dataset: VMDataset, model: VMModel, methods = None, target_recall = None)
```
...
```python
def HyperParametersTuning(model: VMModel, dataset: VMDataset, param_grid: dict, scoring: Union = None, thresholds: Union = None, fit_params: dict = None)
```
...
```python
def KMeansClustersOptimization(model: VMModel, dataset: VMDataset, n_clusters: Union = None)
```
...
```python
def OverfitDiagnosis(model: VMModel, datasets: List, metric: str = None, cut_off_threshold: float = {'cls': 'ExprName', 'name': 'DEFAULT_THRESHOLD'})
```
...
```python
def PermutationFeatureImportance(model: VMModel, dataset: VMDataset, fontsize: Union = None, figure_height: Union = None)
```
...
```python
def PopulationStabilityIndex(datasets: List, model: VMModel, num_bins: int = 10, mode: str = 'fixed')
```
...
```python
def RobustnessDiagnosis(datasets: List, model: VMModel, metric: str = None, scaling_factor_std_dev_list: List = {'cls': 'ExprName', 'name': 'DEFAULT_STD_DEV_LIST'}, performance_decay_threshold: float = {'cls': 'ExprName', 'name': 'DEFAULT_DECAY_THRESHOLD'})
```
...
```python
def ScoreProbabilityAlignment(model: VMModel, dataset: VMDataset, score_column: str = 'score', n_bins: int = 10)
```
...
```python
def TrainingTestDegradation(datasets: List, model: VMModel, max_threshold: float = 0.1)
```
...
```python
def WeakspotsDiagnosis(datasets: List, model: VMModel, features_columns: Union = None, metrics: Union = None, thresholds: Union = None)
```
...
```python
def RegressionModelForecastPlot(model: VMModel, dataset: VMDataset, start_date: Union = None, end_date: Union = None)
```
...
```python
def adj_r2_score(actual: , predicted: , rowcount: int, featurecount: int)
```
...
```python
def build_test_result(outputs: Union, test_id: str, test_doc: str, inputs: Dict, params: Union, title: Optional = None)
```
...
```python
def run_test(test_id: Union = None, name: Union = None, unit_metrics: Union = None, inputs: Union = None, input_grid: Union = None, params: Union = None, param_grid: Union = None, show: bool = True, generate_description: bool = True, title: Optional = None, post_process_fn: Union = None, kwargs = {}) -> TestResult
```
...
- **param_grid (Union[Dict[str, List[Any]], List[Dict[str, Any]]], optional)**: For comparison tests, either: - Dict mapping parameter names to lists of values (creates Cartesian product) - List of parameter dictionaries to test
- **inputs (Dict[str, Any], optional)**: Test inputs (models/datasets initialized with vm.init_model/dataset)
- **input_grid (Union[Dict[str, List[Any]], List[Dict[str, Any]]], optional)**: For comparison tests, either: - Dict mapping input names to lists of values (creates Cartesian product) - List of input dictionaries to test
...
```python
def as_df(series_or_frame: Union) -> {'cls': 'ExprAttribute', 'values': [{'cls': 'ExprName', 'name': 'pd'}, {'cls': 'ExprName', 'name': 'DataFrame'}]}
```
...
```python
def create_figure(figure: Union, key: str, ref_id: str) -> Figure
```
...
####### remove_figure()
```python
def remove_figure(self, index = 0):
```
...
```python
def check_for_sensitive_data(data: , inputs: List)
```
...
```python
def figures_to_widgets(figures: List) -> list
```
...
```python
def tables_to_widgets(tables: List)
```
...
```python
def update_metadata(content_id: str, text: str, _json: Union = None)
```
