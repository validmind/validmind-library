...
from validmind.tests.model_validation.ModelPredictionResiduals import (
...
class TestModelPredictionResiduals(unittest.TestCase):
    def setUp(self):
        ...
        y_true = 2 * X.ravel() + np.random.normal(
            0, 1, 100
        )  # Linear pattern with noise

        # Create DataFrame
        self.df = pd.DataFrame({"feature": X.ravel(), "target": y_true}, index=dates)
    ...
    def test_return_structure(self):
        """Test if function returns expected structure (DataFrame, two figures, RawData)."""
        result = ModelPredictionResiduals(self.vm_dataset, self.vm_model)

        # Should return a tuple of (DataFrame, Figure, Figure, RawData)
    ...
    def test_date_filtering(self):
        ...
        result_df = ModelPredictionResiduals(
            self.vm_dataset, self.vm_model, start_date=start_date, end_date=end_date
        )[0]
    ...
    def test_p_value_threshold(self):
        ...
        summary_df = ModelPredictionResiduals(
            self.vm_dataset, self.vm_model, p_value_threshold=custom_threshold
        )[0]
