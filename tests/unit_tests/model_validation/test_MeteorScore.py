...
from validmind.tests.model_validation.MeteorScore import MeteorScore
from validmind import RawData


class TestMeteorScore(unittest.TestCase):
    def setUp(self):
        ...
        # Create predictions with varying levels of similarity
        predicted_texts = [
            "The brown fox quickly jumps over a lazy dog.",  # High similarity
            "A long journey starts with just one step.",  # Medium similarity
            "That is indeed the question to be or not.",  # Medium similarity
            "Not everything that shines is golden.",  # Low similarity
            "Actions are stronger than mere words.",  # Medium similarity
        ]
    ...
    def test_returns_tuple(self):
        ...
        result = MeteorScore(self.vm_dataset, self.vm_model)
    ...
    def test_metrics_dataframe(self):
        ...
        result_df = MeteorScore(self.vm_dataset, self.vm_model)[0]
    ...
    def test_figures_properties(self):
        ...
        _, *figures, _ = MeteorScore(self.vm_dataset, self.vm_model)
    ...
    def test_identical_texts(self):
        ...
        result_df = MeteorScore(vm_dataset_identical, self.vm_model)[0]

        # For identical texts, METEOR score should be very close to 1.0
        self.assertGreater(result_df["Mean Score"].iloc[0], 0.99)
        self.assertLess(result_df["Mean Score"].iloc[0], 1.01)
