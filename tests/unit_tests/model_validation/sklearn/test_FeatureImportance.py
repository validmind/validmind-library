...
class TestFeatureImportance(unittest.TestCase):
    ...
    def test_returns_dataframe_and_rawdata(self):
        # Run the function
        results_df, raw_data = FeatureImportance(self.vm_dataset, self.vm_model)
    ...
    def test_feature_importance_ranking(self):
        ...
        results_df, raw_data = FeatureImportance(
            self.vm_dataset, self.vm_model, num_features=4
        )

        # Get feature names and scores
        features = []
        for i in range(1, 5):
            feature_info = results_df[f"Feature {i}"].iloc[0]
    ...
    def test_num_features_parameter(self):
        ...
        for num_features in [2, 3, 4]:
            ...
            feature_columns = [
                col for col in results_df.columns if col.startswith("Feature")
            ]
    ...
    def test_feature_importance_scores(self):
        ...
        first_feature = results_df["Feature 1"].iloc[0]
